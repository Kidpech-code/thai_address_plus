...
@immutable
class HierarchyNode {
  const HierarchyNode({required this.pcode, required this.nameTh, required this.nameEn, this.region, this.zipCode});
  final String pcode;
  final String nameTh;
  final String nameEn;
  final String? region;
...
@immutable
class ReverseMatch {
  const ReverseMatch({required this.pcode, required this.nameTh, required this.nameEn, required this.level});
  final String pcode;
  final String nameTh;
  final String nameEn;
  final String level;
...
class ReverseResult {
  ...
  factory ReverseResult.fromJson(Map<String, dynamic> j) {
    ...
    HierarchyNode? read(String k) {
      final v = hier[k];
      return v is Map ? HierarchyNode.fromJson(v.cast<String, dynamic>()) : null;
    }

    return ReverseResult(
      lng: (j['lng'] as num?)?.toDouble() ?? 0,
      lat: (j['lat'] as num?)?.toDouble() ?? 0,
      match: ReverseMatch.fromJson((j['match'] as Map?)?.cast<String, dynamic>() ?? const {}),
